...
function [display_t4,display_t3,display_tsh] = thyrosim_oral_repeat_ben5(fitting_index,current_iter,tspans,patient_param,initial_condition,T4doses,T3doses)

    % Initialize
    T4_init = initial_condition(1);
    T3_init = initial_condition(2);
    Tsh_init = initial_condition(3);
    [ic,dial] = init(patient_param, T4_init, T3_init, Tsh_init);
    inf1 = 0; % For oral doses, infs are 0
    inf4 = 0; % For oral doses, infs are 0

    % Some needed vars for plotting
    t_last = 0;
    T4max = 0;
    T3max = 0;
    TSHmax = 0;

    %define the return vectors that store the desired T4, T3, and TSH values. 
    ...
    % store initial T4, T3, TSH values before simulation
    ...
    for i=1:length(tspans)
        ...
        phase = mod(t_last, 24);
        [t,q] = thyrosim_core_ben5(ic,dial,inf1,inf4,tspans(i,:),fitting_index,current_iter,patient_param,phase); % Run simulation
        ...
        %record t4/t3/tsh at the end of the simulation
...
function graph(t,q)
    ...
    %{
    if strcmp(plot_group, 'jonklass_all')
