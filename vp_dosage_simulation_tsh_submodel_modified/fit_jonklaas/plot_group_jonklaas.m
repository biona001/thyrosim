function plot_group_jonklaas()
    ...
    for i=1:size(index, 2)
        ...
        %conversion factors for new thyrosim
        %{
        T4max = max(total_q(:,1));
        T3max = max(total_q(:,4));
        ...
        %conversion factors for old thyrosim
        T4max = max(total_q(:,1));
        T3max = max(total_q(:,4));
        TSHmax = max(total_q(:,7));
        T4conv  = 777/3.2;         % mcg/L
        T3conv  = 651/3.2;         % mcg/L
        TSHconv = 5.6/5.2;       % mU/L
        y1 = total_q(:,1)*T4conv;     % T4
        y2 = total_q(:,4)*T3conv;     % T3
        y3 = total_q(:,7)*TSHconv;    % TSH
        t  = total_time/24;           % Convert time to days
    ...
    saveas(myfig, ['./worspace/', name, '.png'])
end